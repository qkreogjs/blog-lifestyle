  ...
  return (
    <div>
      ...
      <section style={{ borderBottom: "2px solid #FDE8D0" }} className="mb-12 pb-10">
        ...
        {featured ? (
          <Link href={`/en/blog/${featured.slug}`} className="group block">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              ...
              <div>
                ...
                <span style={{ color: "var(--color-primary)" }} className="text-sm font-semibold">Read more →</span>
              </div>
            </div>
          </Link>
        ) : (
          <div className="text-center py-16">
            ...
            <a href="/en/blog" style={{ backgroundColor: "var(--color-primary)", color: "#fff" }} className="inline-block mt-4 px-6 py-2.5 rounded-full text-sm font-semibold">View All Articles →</a>
          </div>
        )}
      </section>

      {/* 2-column grid - remaining posts */}
      {restPosts.length > 0 && (
        <section>
            ...
            {restPosts.map(post => (
              <article key={post.slug} className="group">
                <Link href={`/en/blog/${post.slug}`}>
                  ...
                  <time style={{ color: "#A8A29E" }} className="text-xs italic">{new Date(post.publishedAt).toLocaleDateString("en-US")}</time>
                </Link>
              </article>
            ))}
          </div>
